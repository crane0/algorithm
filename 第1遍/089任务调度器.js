...
var leastInterval = function(tasks, n) {
  ...
  let max = 0 // 出现最高的次数
  let maxCount = 0 // 最高频率的个数，因为可能是 3A 3B
  const baseIndex = 65 // 'A'.charCodeAt()

  for (const t of tasks) {
    const currentIndex = t.charCodeAt() - baseIndex
    countArr[currentIndex]++
    if (max === countArr[currentIndex]) {
      maxCount++
    } else if (max < countArr[currentIndex]) {
      max = countArr[currentIndex]
      maxCount = 1
    }
  }
  ...
  const enableTasks = tasks.length - max * maxCount
  const spaces = Math.max(0, emptySlots - enableTasks)
